...
export function mainPartUri(pkg: OpcPackage): string {
  const relationship = pkg.rootRelationships.find((item) => item.type.endsWith(OFFICE_DOCUMENT_REL));
...
export const relatedPartUri = (pkg: OpcPackage, sourceUri: string, relationshipId: string): string | undefined => {
  const relationship = pkg.parts.get(sourceUri)?.relationships.find((item) => item.id === relationshipId);
