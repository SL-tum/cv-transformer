...
export function contentTypeFor(uri: string, types: ContentTypes): string {
  ...
  return extension
    ? (types.defaults[extension] ?? "application/octet-stream")
...
export function ensureContentType(
  ...
  if (
    preferDefault &&
    extension &&
    (!types.defaults[extension] || types.defaults[extension] === contentType)
  )
    types.defaults[extension] = contentType;
  else if (!extension || types.defaults[extension] !== contentType)
    types.overrides[uri] = contentType;
  delete types.rawXml;
}
export function removeContentType(types: ContentTypes, partUri: string): void {
  delete types.overrides[normalizePartUri(partUri)];
  delete types.rawXml;
}
